(function() {
  var example_passages, numCols2Ids, presets,
  ...
  $(function() {
    ...
    $("input[name=preset]").click(function() {
      ...
      return $('input[name=ws]').val(presets[preset]['ws']);
    });
    $("#exercises").hide();
    $('#reset').click(reset);
    form = $("form")[0];
    form.onsubmit = function() {
      var cols, i, len, logograms, passage, temp_word, textdir, word, word_ar, word_count, wpc, wpm, ws;
      ...
      col_ar = numCols2Ids[cols].slice(0);
      if (textdir === 'rtl') {
        col_ar.reverse();
      }
      ...
      passage_ar = [];
      word_count = 0;
      for (i = 0, len = word_ar.length; i < len; i++) {
        word = word_ar[i];
        if (word_count >= wpc && temp_word.length >= ws * wpc) {
    ...
    for (title in example_passages) {
      if (!hasProp.call(example_passages, title)) continue;
      paragraph = example_passages[title];
